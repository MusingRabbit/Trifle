...
namespace tfl
{
  class VectorHelper
  {
    ...
    public:
      ...
      static UIntPoint3 Vec3ToUIntPoiunt3(const glm::vec3 v)
      {
          return UIntPoint3((unsigned int)v.x, (unsigned int)v.y, (unsigned int)v.z); 
      }

      static glm::vec3 UIntPoint3ToVec3(const UIntPoint3& v)
      {
          return glm::vec3((float)v.x, (float)v.y, (float)v.z); 
      }

      static Point3 Vec3ToPoiunt3(const glm::vec3 v)
      {
          return Point3((unsigned int)v.x, (unsigned int)v.y, (unsigned int)v.z); 
      }

      static glm::vec3 Point3ToVec3(const Point3& v)
      {
          return glm::vec3((float)v.x, (float)v.y, (float)v.z); 
      }
  };
